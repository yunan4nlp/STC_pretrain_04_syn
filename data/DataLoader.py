...
def read_training_corpus(file_src_path, file_tgt_path, min_src_length, min_tgt_length, max_src_length, max_tgt_length):
    ...
    for index in range(length_src):
        if len(src_data_org[index]) > max_src_length or len(tgt_data_org[index]) > max_tgt_length:
            continue
        if len(src_data_org[index]) < min_src_length or len(tgt_data_org[index]) < min_tgt_length:
            continue
